...
const TeamsHero = ({ team }: TeamsHeroProps) => {
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-titanes-700 via-titanes-800 to-titanes-900 px-6 pb-14 pt-34 text-white sm:pt-40">
      ...
      <div className="relative mx-auto flex w-full max-w-6xl flex-wrap items-end justify-between gap-8">
        <div className="max-w-2xl space-y-3">
          <p className="text-xs font-semibold tracking-[0.2em] text-titanes-100 uppercase">Squad Overview</p>
          <h1 className="text-4xl leading-tight font-black sm:text-6xl">{team.title}</h1>
          <p className="max-w-xl text-base text-titanes-100/90 sm:text-lg">{team.subtitle}</p>
        </div>
...
const StatCard = ({ label, value }: StatCardProps) => (
  <article className="rounded-xl border border-white/20 bg-white/10 px-4 py-3 backdrop-blur-sm">
    <p className="text-xs tracking-[0.14em] text-titanes-100/80 uppercase">{label}</p>
