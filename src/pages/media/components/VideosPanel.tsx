    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {items.map((item) => (
        <article key={item.id} className="overflow-hidden rounded-2xl border border-primary-100 bg-white shadow-sm">
          {item.link ? (
            <a href={item.link} target="_blank" rel="noreferrer" className="group relative block">
              <img src={item.imageSrc} alt={item.title} className="h-44 w-full object-cover object-center" />
