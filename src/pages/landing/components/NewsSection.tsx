				...
				<div className="mt-10 grid gap-5 md:grid-cols-3">
					{NEWS.map((item) => (
						<article key={item.title} className="rounded-2xl border border-white/10 bg-white/5 p-6">
							...
							<p className="mt-3 text-sm leading-relaxed text-primary-100/90">{item.description}</p>
							<a
								href={item.href}
								className="mt-5 inline-flex text-xs font-semibold uppercase tracking-[0.12em] text-primary-200"
							>
								Leer nota
							</a>
						</article>
