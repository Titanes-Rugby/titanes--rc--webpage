...
describe('<TeamsPage />', () => {
  it('renders players tab content for explicit players route', () => {
    ...
    expect(screen.getByRole('heading', { name: /Titanes Primera/i })).toBeInTheDocument();
    expect(screen.getByText(/6 players found/i)).toBeInTheDocument();
  ...
  it('falls back to players tab when tab is invalid', () => {
    ...
    expect(screen.getByRole('heading', { name: /Titanes Femenino/i })).toBeInTheDocument();
    expect(screen.getByText(/6 players found/i)).toBeInTheDocument();
  });
